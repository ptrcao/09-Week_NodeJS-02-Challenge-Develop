...
var emailValidate = (email) => { 
    if(EmailValidator.validate(email)){
        email = email;
        return true}
    else{return 'Invalid email format - please try again to enter a valid email address.'}
}
...
async function promptAndValidateGHRepo(message) {
    const answer = await inquirer.prompt([{ type: 'input', name: 'ghRepo', message }])
    ...
    if (data.message) {
      if (data.message === "Not Found") {
        return promptAndValidateGHRepo("Repository was not found, please provide a valid, existing repository name.")
      }
    }
    // else {
  ...
  const ghMessage = "What is your GitHub username? (This is needed for Shields badges and is used to retrieve code size, repo size, repo file count and license type)";
  
  const ghMessage2 = "What is the GitHub repository name? (This is needed for Shields badges and is used to retrieve code size, repo size, repo file count)";
  ...
  const listB = [
    ...
    },
    {
        type: "input",
        name: "usage",
        message: "Please provide usage instructions.  We recommend inserting screenshots manually later.",
    },
    {
        type: "input",
        name: "tests",
    ...
    },
    {
        ...
        choices: ['MIT', 'Apache', 'GPL', new inquirer.Separator(), 'No license', 'Other']
    },
    {
        type: "input",
        name: "licenseOther",
        message: 'Please specify the "Other" type of license pertains to this project?',
...
![license](https://img.shields.io/static/v1?label=license&message=${license().replace(/ /g,"_")}&color=blue&style=for-the-badge&logo=appveyor)
...
function license(){if(answersB.license === "Other"){
    return answersB.licenseOther} else {return answersB.license}
}
  ...
  async function prompts() {
    ...
    console.log(`Repo URL: https://api.github.com/repos/${ghUsername}/${ghRepoName}`)
